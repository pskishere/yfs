...
const TradingViewChart: React.FC<TradingViewChartProps> = ({
   ...
   */
  const parseTime = (timeStr: string): Time | null => {
    try {
      // 尝试解析 "YYYY-MM-DD" 格式
      ...
      if (parts.length === 3) {
        ...
        const day = parseInt(parts[2].split(' ')[0]); // 处理可能包含的时间部分
        
        // 检查日期有效性
        if (year > 1900 && month >= 0 && month < 12 && day > 0 && day <= 31) {
          const date = new Date(year, month, day);
          // 如果包含时间部分（分钟/小时），需要更精确的解析
          if (timeStr.includes(':')) {
    ...
    const updateData = () => {
      ...
      try {
        ...
        const formattedData = validCandles.map(candle => ({
          ...
          low: candle.low,
          close: candle.close,
        })).filter((item): item is { time: Time; open: number; high: number; low: number; close: number } => item.time !== null);

        // 按时间排序，防止数据乱序导致 crash
        ...
        if (volumeSeriesRef.current) {
          const volumeData = validCandles.map(candle => ({
            ...
            color: candle.close >= candle.open 
              ? '#26a69a80'
              : '#ef535080',
          })).filter((item): item is { time: Time; value: number; color: string } => item.time !== null);
          ...
          for (const item of volumeData) {
             const t = item.time as number;
             if (!seenVolTimes.has(t)) {
               seenVolTimes.add(t);
               uniqueVolume.push(item);
             }
          }
  ...
  useEffect(() => {
    ...
    maPeriods.forEach(({ period, color, visible }) => {
      ...
      for (let i = period - 1; i < candles.length; i++) {
        ...
        const time = parseTime(candles[i].time);
        if (time !== null) {
          maData.push({
            time: time,
            value: avg,
  ...
  useEffect(() => {
    ...
    if (indicators.bb_upper_series && indicators.bb_middle_series && indicators.bb_lower_series) {
      ...
      const upperData = upperSeries.map((value, idx) => ({
        ...
        value: value,
      })).filter((item): item is { time: Time; value: number } => item.time !== null);
    ...
    // 向后兼容：如果没有历史序列，使用旧的静态值方式
      ...
      })).filter((item): item is { time: Time; upper: number; middle: number; lower: number } => item.time !== null);
